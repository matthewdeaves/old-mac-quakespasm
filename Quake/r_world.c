...
/*
================
R_DrawWorld_WaterDepthPrepass -- PPC port (Round v6)

Writes water-surface depth into the depth buffer BEFORE entities are
drawn so items in lower rooms / connected leaves correctly fail the
depth test against any water-flagged BSP surface, fixing the X-ray
artifact at r_wateralpha < 1.

Without this, translucent water disables depth-write
(R_BeginTransparentDrawing), so item entities drawn in step 4 of
R_RenderScene have nothing to depth-test against from water surfaces.
Items behind container floors / SURF_DRAWTURB stairs paint into the
framebuffer; the later water alpha pass blends with them, giving the
"see items in lower room through stairs" artifact.

Color writes are masked, so the prepass is invisible. Vertex
positions match exactly what the actual water color passes submit
(DrawGLPoly + DrawWaterPoly + the GLSL shader path all use the same
glpoly_t verts), so depth values are bit-identical. The water color
pass uses GL_LEQUAL (set by R_DrawWorld_Water) so equal-depth water
surfaces still render against the prepass values.

Skipped automatically when no liquid alpha is < 1, since the standard
opaque-water pipeline doesn't have this issue.
================
*/
void R_DrawWorld_WaterDepthPrepass (void)
{
	int		i;
	msurface_t	*s;
	texture_t	*t;
	...
	if (!model || r_drawflat_cheatsafe || r_lightmap_cheatsafe)
		return;

	// Skip when all liquids are opaque (default cvar values).
	// r_lavaalpha/r_telealpha/r_slimealpha == 0 means "follow r_wateralpha".
	{
		float wa = r_wateralpha.value;
		float la = (r_lavaalpha.value <= 0.0f)  ? wa : r_lavaalpha.value;
		float ta = (r_telealpha.value <= 0.0f)  ? wa : r_telealpha.value;
		float sa = (r_slimealpha.value <= 0.0f) ? wa : r_slimealpha.value;
		if (wa >= 1.0f && la >= 1.0f && ta >= 1.0f && sa >= 1.0f)
			return;
	}

	glColorMask (GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);
	// glDepthMask is GL_TRUE here (default state at this point in the
	// frame). glDepthFunc is GL_LESS (Quake default), which is fine for
	// the prepass — we're writing into a depth buffer that has only the
	// already-drawn opaque world geometry, so nothing equal-depth.

	for (i = 0; i < model->numtextures; i++)
	{
		t = model->textures[i];
		if (!t || !t->texturechains[chain_world]
		      || !(t->texturechains[chain_world]->flags & SURF_DRAWTURB))
			continue;
		for (s = t->texturechains[chain_world]; s; s = s->texturechain)
		{
			// The actual color paths iterate s->polys->next (subdivided
			// warp polys, present after gl_warp.c subdivision at load
			// time). Match that exactly so depth values match the color
			// pass bit-for-bit. Fall through to s->polys if no
			// subdivision happened (defensive — should be rare).
			p = (s->polys && s->polys->next) ? s->polys->next : s->polys;
			while (p)
			{
				DrawGLPoly (p);
				p = p->next;
