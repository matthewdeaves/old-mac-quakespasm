...
| `deploy.sh <machine>` | assemble `Quakespasm.app` bundle (fat binary + codecs + SDL + nib + icon + Info.plist + per-arch and per-machine autoexec cfgs in `Contents/Resources/`) and rsync to `<machine>:/Applications/QuakeSpasm/`, the same folder deploy-dmg.sh installs releases into (#55). Same bundle for every machine, host.c picks the right slice + per-machine cfg at boot. |
