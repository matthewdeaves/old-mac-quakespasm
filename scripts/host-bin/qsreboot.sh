...
#
# Tier 1: `sudo -n /sbin/reboot` -- direct kernel reboot, works even if
...
#
# Deployed to ~/bin/qsreboot.sh on G3 / G4 / G4mini / Lion via
# scripts/install-host-tools.sh. Used by the orchestration host
# (Ubuntu) to recover machines remotely without needing a keyboard.
